...
def precision(value: int) -> None:
    """
    Set the max length of fractional part of a real number. The default value is 6.
    Each coordinate number of the pathdata is calculated as a Python float value,
    and formatted with that length.
    """
...
def pathdata_from_string(src: str) -> PathData:
    ...
    is a value of the `d` property of the SVG-path element.
    """
...
def transform_from_string(src: str) -> Transform:
    ...
    `svgpdtools.Transform` object. The syntax of transform functions are the same as
    the SVG `transform` attribute, differ from the CSS property.
    """
